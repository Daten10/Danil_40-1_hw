...
def start_game():
    ...
    last_count = config('MY_MONEY', cast=int)
    while(exit!=0):
        ...
        if exit == 0:
            if money > config('MY_MONEY', cast=int):
                print(f'Итог, вы в плюсе на {money - last_count}$')
            elif money < config('MY_MONEY', cast=int):
                print(f'Итог, вы в минусе на {last_count - money}$')
